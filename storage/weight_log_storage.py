...
class WeightLogStorage():
    ...
    def save(self, weight_log):
        self.__table.put_item(Item=weight_log.to_json())

    def remove(self, user_id, timestamp):
        timestamp = (timestamp.isoformat()
                     if isinstance(timestamp, datetime)
                     else str(timestamp))
        params = dict(user_id=user_id, timestamp=timestamp)
        self.__table.delete_item(Key=params)
    ...
    def get(self, user_id, timestamp):
        ...
        raise KeyError(f'Could not find weight log for user {user_id} with timestamp {timestamp}.')
    ...
    def search(self, user_id, **kwargs):
        ...
        Args:
            ...
            ascending (bool): Sort order of the logs using timestamp sort key. Defaults to True.
